...
# Define model
@model function mymodel(time, X, seasonality, exog)
    # prior
    phi_1 ~ Normal(0, 1)
    phi_2 ~ Normal(0, 1)
    beta ~ Normal(0, 1)
    ...
    X[2] ~ Normal(0, sigma)
    for t in 3:time
        mu = phi_1 * X[t-1] + phi_2 * X[t-2] + beta * exog[t] + gamma * cos(2 * pi * seasonality[t])
        X[t] ~ Normal(mu, sigma)
    end
end
...
# Calculate the prediction confidence interval
function prediction_interval(y_pred, alpha=0.05)
    lower = quantile(y_pred, alpha / 2)
    upper = quantile(y_pred, 1 - alpha / 2)
    return (lower, upper)
end

# ... (your existing code)

# Calculate the MAPE and MSE for the forecast
mape_val = mape(X_data_2018, X_fcst_mean)
mse_val = mse(X_data_2018, X_fcst_mean)

# Calculate the prediction confidence interval
conf_interval = prediction_interval(X_fcst_mean)

# Print the MAPE, MSE, and confidence interval
println("Mean Absolute Percentage Error (MAPE): ", mape_val)
println("Mean Squared Error (MSE): ", mse_val)
println("Prediction Confidence Interval: ", conf_interval)