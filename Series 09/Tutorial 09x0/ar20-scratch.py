...
class MyDataset1(Dataset):
    ...
    def __init__(self):
        ...
        array = np.loadtxt('ClrCdtn_Train')
...
datasetTrain, datasetVal = train_valid_split(dataset1)  # training set & validation set
train_loader = DataLoader(dataset=datasetTrain, batch_size=100, shuffle=True) #训练集
validation_loader = DataLoader(dataset=datasetVal,batch_size=20, shuffle=True)                            #验证集 for hyperparameter tuning
test_loader = DataLoader(dataset=dataset2)



num_iterations = 100 #需要调整 stopping criterion
loss = 0
loss_train_epoch = np.zeros(num_iterations)
loss_valid_epoch = np.zeros(num_iterations)
lam = 0
fc1w_mu_value = np.zeros((num_iterations,19))
# normalizer_train = 0
...
for j in range(num_iterations):
        ...
        for batch_id, data in enumerate(train_loader):
            ...
            guide_trace = poutine.trace(guide).get_trace(data[0], data[1])
            model_trace = poutine.trace(poutine.replay(model, trace=guide_trace)).get_trace(data[0], data[1])
            for node in guide_trace.nodes.values():
                 if node["name"] == "fc1w_mu":
                     fc1w_mu_value[j][batch_id] = node["value"].unconstrained()[0][0]
        ...
        loss_valid_epoch[j] = loss_valid / normalizer_valid*0.8

# np.savetxt('BNN_L2_trainloss',loss_train_epoch)
np.savetxt('BNN_trainloss',loss_train_epoch)
...
plt.plot(np.arange(0,num_iterations),loss_valid_epoch, label = 'Validation Loss')
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.legend()
plt.show()

#Plot how the mu changes
# fc1w_mu = np.mean(fc1w_mu_value, axis=1)
# sns.set_style("whitegrid")
# plt.title('Mean of W1')
# plt.plot(np.arange(0,num_iterations),fc1w_mu, label = 'Mean of Weight 1')
# plt.xlabel('Epoch')
# plt.ylabel('Weight')
# plt.legend()
# plt.show()
#
# plt.title(' Distribution of W1 Mean')
# sns.kdeplot(fc1w_mu, shade=True)
# plt.show()
